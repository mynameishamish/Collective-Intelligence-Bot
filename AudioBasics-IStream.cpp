...
//for recording time speaking
#include <time.h>
#include <iostream>
		double person1Time = 0;
	double person2Time = 0;
	double person3Time = 0;
	double silentOrNoisey = 0;
...
LRESULT CALLBACK CAudioBasics::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_TIMER:
        ...
        {    ProcessAudio();
			float beamAngle = 180.f * m_fBeamAngle/static_cast<float>(M_PI);
			float beamConfidence = m_fBeamAngleConfidence;
			float confThreshold = 1.00;

			//record individual speaking time///////////////////////////////////
			if (beamAngle >= -50 && beamAngle <= -17 && beamConfidence >= confThreshold){
				person1Time = person1Time + (50* pow(10, -3));
			}
			else if (beamAngle > -17 && beamAngle <= 17 && beamConfidence >= confThreshold){
				person2Time = person2Time + (50* pow(10, -3));
			}
			else if (beamAngle > 17 && beamAngle <= 50 && beamConfidence >= confThreshold){
				person3Time = person3Time + (50* pow(10, -3));
			}
			else if (beamConfidence < confThreshold){
				silentOrNoisey = silentOrNoisey + (50* pow(10, -3));
			}
			/////////////////////////////////////////////////////////////////////
        }
        else if(wParam == cEnergyRefreshTimerId)
		{
			...
			if ((person1Time < (person2Time)) && (person1Time < (person3Time))){
				String^ answer;
				String^ portName;
				String^ st;
				int baudRate = 115200;
			...
			else if ((person2Time < (person1Time)) && (person2Time < (person3Time))){
				String^ answer;
				String^ portName;
				String^ st;
				int baudRate = 115200;
			...
			else if ((person3Time < (person1Time)) && (person3Time < (person2Time))){
				String^ answer;
				String^ portName;
				String^ st;
				int baudRate = 115200;
    ...
    case WM_CLOSE:
		...
		//print out individual speaking times//////////////////////////////////////////////////
		sprintf(person1,"Person 1 spoke %f seconds\n",person1Time);
		OutputDebugStringA(person1);

		sprintf(person2, "Person 2 spoke %f seconds\n",person2Time);
		OutputDebugStringA(person2);
		
		sprintf(person3, "Person 3 spoke %f seconds\n",person3Time);
		OutputDebugStringA(person3);
		
		sprintf(other, "No speaking or multiple speakers occured for %f seconds\n",silentOrNoisey);
...
{  float beamAngle = 180.f * m_fBeamAngle/static_cast<float>(M_PI);
	float beamConfidence = m_fBeamAngleConfidence;
	float confThreshold = 1.00;
	double speakingThresh = 0.5;

	//display who is speaking visually in real time////////////////////////////
	if (beamAngle >= -50 && beamAngle <= -17 && beamConfidence >= confThreshold){
		StringCbPrintf(m_szBeamText, sizeof(m_szBeamText), L"Beam Angle: %2.0f deg - Person 1 speaking, Beam Angle Confidence: %1.2f", 180.0f * m_fBeamAngle/static_cast<float>(M_PI), m_fBeamAngleConfidence);
		 SetStatusMessage(m_szBeamText);
	}

	else if (beamAngle > -17 && beamAngle <= 17 && beamConfidence >= confThreshold){
		StringCbPrintf(m_szBeamText, sizeof(m_szBeamText), L"Beam Angle: %2.0f deg - Person 2 is speaking, Beam Angle Confidence: %1.2f", 180.0f * m_fBeamAngle/static_cast<float>(M_PI), m_fBeamAngleConfidence);
		SetStatusMessage(m_szBeamText);
	}

	else if (beamAngle > 17 && beamAngle <= 50 && beamConfidence >= confThreshold){
		StringCbPrintf(m_szBeamText, sizeof(m_szBeamText), L"Beam Angle: %2.0f deg - Person 3 is speaking, Beam Angle Confidence: %1.2f", 180.0f * m_fBeamAngle/static_cast<float>(M_PI), m_fBeamAngleConfidence);
		SetStatusMessage(m_szBeamText);
	}
	
	else if (beamConfidence < confThreshold){
		StringCbPrintf(m_szBeamText, sizeof(m_szBeamText), L"Beam Angle: %2.0f deg - noone is speaking, Beam Angle Confidence: %1.2f", 180.0f * m_fBeamAngle/static_cast<float>(M_PI), m_fBeamAngleConfidence);
		SetStatusMessage(m_szBeamText);
	}
	///////////////////////////////////////////////////////////////////////////
}